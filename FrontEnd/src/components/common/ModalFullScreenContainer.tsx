...
function ModalFullScreenContainer({ children, title, onClose, maxWidthClassName, bgClassName }: ModalWrapperProps) {
    ...
    return ReactDOM.createPortal(
        <div className="bg-modal fixed bottom-0 left-0 right-0 top-0 z-[2000] flex h-dvh w-screen justify-center bg-gray-800 bg-opacity-40 pt-4">
            <div
                className={`animate-slide-up flex w-full scroll-smooth ${maxWidthClassName ? maxWidthClassName : 'max-w-screen-tablet'} flex-col rounded-t-lg border-x border-t ${bgClassName ? bgClassName : 'bg-white'}`}
            >
