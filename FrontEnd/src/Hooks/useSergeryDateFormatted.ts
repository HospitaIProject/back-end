...
const useSurgeryDayFormat = (date: string, writeDate?: string) => {
    if (!date)
        return {
            diffDay: '',
        }; // date가 없으면 빈 문자열 반환
    const today = dayjs().startOf('day'); // 현재 날짜의 시간을 제거
    const surgeryDay = dayjs(date).startOf('day'); // 수술 날짜의 시간을 제거

    const diffDay = surgeryDay.diff(today, 'day'); // 수술 날짜와 현재 날짜의 차이

    if (writeDate) {
        const writeDay = dayjs(writeDate).startOf('day'); // 글쓴 날짜의 시간을 제거
        const writeDiffDay = writeDay.diff(surgeryDay, 'day'); // 글쓴 날짜와 수술 날짜의 차이
