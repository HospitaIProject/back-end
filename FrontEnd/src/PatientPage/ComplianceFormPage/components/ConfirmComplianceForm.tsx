...
function ConfirmComplianceForm({ formValues, prevValues, todayValues, postValues, onSubmit, existFields }: Props) {
    ...
    return (
        <>
            ...
            <div
                className={`${dateStatus === 'PREV' ? 'grid' : 'hidden'} w-full grid-cols-1 flex-col gap-3 px-4 pt-4 tablet:grid-cols-2 tablet:gap-x-20`}
            >
                {/* 수술전 */}
            ...
            <div
                className={`${dateStatus === 'POST' ? 'grid' : 'hidden'} w-full grid-cols-1 flex-col gap-3 px-4 pt-4 tablet:grid-cols-2 tablet:gap-x-20`}
            >
                {/* 수술후 */}
                <YesOrNoViewButton
